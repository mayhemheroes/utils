...
fuzz_target!(|input: (&[u8], &[u8], &[u8])| {
    ...
    Iso7816::pad(&mut block_16, pos);

    let res = Iso7816::unpad(&block_16).unwrap();
    assert_eq!(res, fst);

    let mut block_32: GenericArray::<u8, U32> = [0xff; 32].into();
    block_32[..pos_2].copy_from_slice(&snd[0..pos_2]);
    Iso7816::pad(&mut block_32, pos_2);

    let res = Iso7816::unpad(&block_32).unwrap();
    assert_eq!(res, snd);

    let mut block_20: GenericArray::<u8, U20> = [0xff; 20].into();
    block_20[..pos_3].copy_from_slice(&thrd[0..pos_3]);
    Iso7816::pad(&mut block_20, pos_3);

    let res = Iso7816::unpad(&block_20).unwrap();
    assert_eq!(res, thrd);
});